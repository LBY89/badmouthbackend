...
complaintsRouter.get("/:id", (request, response, next) => {
  Complaint.findById(request.params.id)
    .then((complaint) => {
      if (complaint) {
...
complaintsRouter.get("/", async (request, response) => {
  const complaints = await Complaint.find({})
    ...
    .populate("comments", { content: 1, complaintId: 1, userId: 1 });
...
complaintsRouter.put("/:id", async (request, response) => {
  ...
  await updatedComplaint.populate("user", {
    firstname: 1,
    lastname: 1,
    email: 1,
    id: 1,
...
complaintsRouter.post("/", async (request, response) => {
  ...
  console.log("n complaint obj", complaint);

  const savedComplaint = await complaint.save();
  await savedComplaint.populate("user", {
    firstname: 1,
    lastname: 1,
    email: 1,
    id: 1,
